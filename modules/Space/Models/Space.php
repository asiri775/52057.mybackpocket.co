...
class Space extends Bookable
{
            ...
            case "on-sale":
                $model_space->where(function ($query) {
                    $query->where(function($subquery) {
                        $subquery->where('discounted_hourly', '>', 0)
                            ->where('discounted_hourly', '<', DB::raw('hourly'));
                    })
                    ->orWhere(function($subquery) {
                        $subquery->where('discounted_daily', '>', 0)
                            ->where('discounted_daily', '<', DB::raw('daily'));
                    })
                    ->orWhere(function($subquery) {
                        $subquery->where('discounted_weekly', '>', 0)
                            ->where('discounted_weekly', '<', DB::raw('weekly'));
                    })
                    ->orWhere(function($subquery) {
                        $subquery->where('discounted_monthly', '>', 0)
                            ->where('discounted_monthly', '<', DB::raw('monthly'));
                    });
                });
                // $model_space->orderByRaw('CASE WHEN discounted_hourly > 0 AND discounted_hourly < hourly THEN 1 ELSE 0 END', 'desc')
                //     ->orderByRaw('CASE WHEN discounted_daily > 0 AND discounted_daily < daily THEN 1 ELSE 0 END DESC')
                //     ->orderByRaw('CASE WHEN discounted_weekly > 0 AND discounted_weekly < weekly THEN 1 ELSE 0 END DESC')
                //     ->orderByRaw('CASE WHEN discounted_monthly > 0 AND discounted_monthly < monthly THEN 1 ELSE 0 END DESC');
                break;
            case "price_high_low":
                // $raw_sql = "CASE WHEN IFNULL( bravo_spaces.sale_price, 0 ) > 0 THEN bravo_spaces.sale_price ELSE bravo_spaces.price END AS tmp_min_price";
                // $model_space->selectRaw($raw_sql);
    ...
    public function addressWithDistance($showWithAddress = true, $explodedCity = false)
    {
        ...
        if(!CodeHelper::isNotEmpty($this->address_unit)){
            $addressMain = $this->address.", ".$this->city.", ".$this->state." ".$this->zip.", ".$this->country;
        }

        $mapLink = 'https://www.google.com/maps/search/?api=1&query=' . trim(urlencode($addressMain));
        if (Session::has('lat') && Session::has('lng')) {
            $lat = Session::get('lat');
            $lon = Session::get('lng');
