...
class UserController extends FrontendController
{
    ...
    public function singleBookingDetails($id) 
    {
        ...
        if ($this->hasPermission('dashboard_vendor_access')) {
            $review = Review::where('vendor_id', $user->id)->where('reference_id', $booking->id)->first();
            return view('User::frontend.bookingDetail-host', compact('booking', 'page_title', 'space', 'customer', 'review'));
        } else {
            $review = Review::where('create_user', $user->id)->where('reference_id', $booking->id)->first();
            return view('User::frontend.bookingDetail', compact('booking', 'page_title', 'space', 'customer', 'review'));
        }
    }
